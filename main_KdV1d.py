...
# solving KdV: u_t + uu_x + 0.0025u_xxx = 0, x in [-1,1] 
# periodic b.c.
def get_parser():
    parser = argparse.ArgumentParser(description='KdV GP solver')
    
    # equation parameters
    parser.add_argument("--nu", type=float, default = 0.0025)
...
def get_GNkernel_train(x,y,wx0,wx1,wxg,wy0,wy1,wyg,d,sigma):
    # wx0 * delta_x + wxg * nabla delta_x + wx1 * nabla Delta delta_x 
    return wx0*wy0*kappa(x,y,d,sigma) + wx0*D_wy_Delta_y_kappa(x,y,d,sigma,wy1) + wy0*D_wx_Delta_x_kappa(x,y,d,sigma,wx1) + D_wx_Delta_x_D_wy_Delta_y_kappa(x,y,d,sigma,wx1,wy1) + wx0*D_wy_kappa(x,y,d,sigma,wyg) + wy0*D_wx_kappa(x,y,d,sigma,wxg) + D_wx_Delta_x_D_wy_kappa(x,y,d,sigma,wx1,wyg) + D_wx_D_wy_Delta_y_kappa(x,y,d,sigma,wxg,wy1) + D_wx_D_wy_kappa(x,y,d,sigma,wxg,wyg)
@jit
def get_GNkernel_train_boundary(x,y,wy0,wy1,wyg,d,sigma):
    return wy0*kappa(x,y,d,sigma) + D_wy_Delta_y_kappa(x,y,d,sigma,wy1) + D_wy_kappa(x,y,d,sigma,wyg)
@jit
...
def get_GNkernel_ux_predict(x,y,wy0,wy1,wyg,d,sigma):
    wxg = 1.0
    return wy0*D_wx_kappa(x,y,d,sigma,wxg) + D_wx_D_wy_Delta_y_kappa(x,y,d,sigma,wxg,wy1) + D_wx_D_wy_kappa(x,y,d,sigma,wxg, wyg)
@jit
def get_GNkernel_uxxx_predict(x,y,wy0,wy1,wyg,d,sigma):
    wx1 = 1.0
    return wy0*D_wx_Delta_x_kappa(x,y,d,sigma,wx1) + D_wx_Delta_x_D_wy_Delta_y_kappa(x,y,d,sigma,wx1,wy1) + D_wx_Delta_x_D_wy_kappa(x,y,d,sigma,wx1,wyg)

def assembly_Theta(X_domain, w0, w1, wg,sigma):
    # X_domain, dim: N_domain*d; 
    # w0 col vec: coefs of Diracs, dim: N_domain; 
    # w1 coefs of Delta Laplacians, dim: N_domain
...
def assembly_Theta_value_predict(X_infer, X_domain, w0, w1, wg, sigma):
    ...
    Theta = onp.zeros((3*N_infer,N_domain))
    ...
    val = vmap(lambda x,y,wy0,wy1,wyg: get_GNkernel_uxxx_predict(x,y,wy0,wy1,wyg,d,sigma))(XiXd0,XiXd1,arr_wy0,arr_wy1,arr_wyg)
