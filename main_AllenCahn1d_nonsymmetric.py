...
@jit
def get_GNkernel_train(x,y,wx0,wx1,d,sigma):
    # wx0 * delta_x + wxg * nabla delta_x + wx1 * Delta delta_x 
    return wx0*kappa(x,y,d,sigma) + wx1*Delta_x_kappa(x,y,d,sigma)
@jit
...
def assembly_Theta(X_domain, w0, w1, sigma):
    ...
    val = vmap(lambda x,y,wx0,wx1: get_GNkernel_train(x,y,wx0,wx1,d,sigma))(XdXd0,XdXd1,arr_wx0,arr_wx1)
    Theta[:N_domain,:N_domain] = onp.reshape(val, (N_domain,N_domain))
    
    return Theta

def assembly_Theta_value_predict(X_infer, X_domain, sigma):
    ...
    for iter_i in range(Nt-1):
        ...
        for iter_step in range(GN_step):
            if time_stepping == "CrankNicolson":
                ...
                rhs= rhs_BE + 10*temp**3
                w0 = 1/dt - 5 + 15*temp**2

            Theta_train = assembly_Theta(cur_X_domain, w0[:, onp.newaxis], w1[:, onp.newaxis],sigma)
            Theta_test = assembly_Theta_value_predict(cur_X_domain, cur_X_domain, sigma)
